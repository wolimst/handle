  ...
  describe('submitGuess(Hangul.Word) -> GuessResult', () => {
    ...
    describe('GuessResult.result should be undefined if the guess is invalid', () => {
      ...
      test('guess is shorter than the answer', () => {
        ...
        expect(wordle.submitGuess(shortGuessInWordList).result).toBeUndefined()
      })

      test('guess is longer than the answer', () => {
        const longGuessInWordList1 = toWord('다람쥐')
        expect(wordle.submitGuess(longGuessInWordList1).result).toBeUndefined()
      })

      test('guess is not in word list', () => {
        const emptyWord = toWord('')
        expect(wordle.submitGuess(emptyWord).result).toBeUndefined()

        const guessNotInWordList = toWord('없닭')
        expect(wordle.submitGuess(guessNotInWordList).result).toBeUndefined()

        const shortGuessNotInWordList = toWord('짧')
        expect(
          wordle.submitGuess(shortGuessNotInWordList).result
        ).toBeUndefined()

        const longGuessNotInWordList = toWord('너무긺')
        expect(
          wordle.submitGuess(longGuessNotInWordList).result
        ).toBeUndefined()
      })
    })
  })
  ...
  describe('wordle game play', () => {
    describe('nGuesses: 4, answer: "정답"', () => {
      ...
      test('submitting guess after a win should have no effect', () => {
        ...
        wordle.submitGuess(toWord(answer))
        expect(wordle.guessResults).toStrictEqual([res1])
        expect(wordle.status).toStrictEqual(WIN)

        wordle.submitGuess(toWord('언덕'))
        expect(wordle.guessResults).toStrictEqual([res1])
        expect(wordle.status).toStrictEqual(WIN)
      })
